...
function Dashboard({ user, children, schedules, handleLogout, setSelectedChild, setSelectedScheduleDate, getSchedule, saveSchedule }) {
...
const launchCelebration = () => {
 ...
 const addQuickActivity = (child, act) => {
  ...
  const already = existing.find(
    a => a.name === act.name && a.time === act.time
  );

  if (already) {

    const msg =
      `"${act.name}" is already in ${child.name}'s schedule!`;

    setQuickMsg(msg);

    speakText(`${act.name} already exists`);
    ...
    const msg =
      `✅ Added "${act.name}" to ${child.name}'s schedule!`;
    ...
    speakText(
      `${act.name} added successfully for ${child.name}`
    );
      ...
      <main className="dashboard-main fade-in">
        ...
        <div className="dash-header">
          <div>
            ...
            <p className="dash-subtitle">
              Welcome back, <strong>{user?.name}</strong>! Here's today's overview.
            </p>
          </div>
        ...
        {children.length > 0 && (
          <div className="quick-add-section">
            ...
            <h2 className="section-title">
              ⚡ Quick Add Activities to Today's Schedules
            </h2>

            <p className="section-sub">
              Click any activity button under a child's avatar to quickly add it to their schedule.
            </p>

            {quickMsg && (
              <div className="quick-msg bounce-in">
                {quickMsg}
              </div>
            )}
        ...
        {children.length === 0 ? (
          ...
          <div className="empty-state">
            ...
            <p>
              Create a child profile to start building daily schedules.
            </p>
                ...
                >
                  ...
                  <div className="child-card-top">
                    ...
                    <div
                      ...
                      style={{
                        background: child.color || 'var(--primary-light)'
                      }}
                    >
                      {child.avatar || '🧒'}
                    </div>
                  ...
                  {pct === 100 && stats.total > 0 && (
                    <div className="all-done-strip">
                      🎉 All tasks completed! Wonderful day!
                    </div>
                  )}
